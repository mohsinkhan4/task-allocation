...
export class UserService {
    ...
    getUser(username) {
        ...
        return this.http.get(url + '/ReleaseDeployment/rest/users?username=' + username); 
    }

    getUsers() {
        const { HOST, PORT } = this.envService.getConnectionDetails();
        const url = (HOST && PORT) ? 'http://' + HOST + ':' + PORT : '';
        return this.http.get(url + '/ReleaseDeployment/rest/users/users'); 
    }

    updateUser(username, securityGroup, addSecurityGroup) {
        const { HOST, PORT } = this.envService.getConnectionDetails();
        const url = (HOST && PORT) ? 'http://' + HOST + ':' + PORT : '';
        return this.http.post(url + '/ReleaseDeployment/rest/users/update', { username, securityGroup, addSecurityGroup });
    }

    fromSGTosg(SG) {
        return this.SG_sg[SG];
    }
    ...
    updateUsers(userData) {
        const { HOST, PORT } = this.envService.getConnectionDetails();
        const url = (HOST && PORT) ? 'http://' + HOST + ':' + PORT : '';
        return this.http.put(url + '/rest/users', { userData });
    }
